...
#     EVERY project. Set this on the inspiring-cat service to patch anything.
#   RAILWAY_TOKEN     — PROJECT token (scoped to one project). Used by the app's
#     own autonomous redeploy; left untouched here. NOTE: when a project token is
#     present it can pin the CLI to that one project, so the VS Code terminal env
#     below deliberately clears RAILWAY_TOKEN and exports only RAILWAY_API_TOKEN.
...
# Pro). If the plan caps effort, the value won't take — verify in the live REPL
# (run `/ultracode`, "set as default", then `cat /root/.claude/settings.json`)
# and replace the value below with exactly what the build writes.
...
echo "[entrypoint] Claude Code CLI settings written — Opus 4.8 + ultracode default, all tools pre-approved."
...
super-agent (this / inspiring-cat) · friday_jarvis2 (voice worker + UI) ·
OpenJarvis-Avengers (worker + UI) · osiris · hermes-agent · n8n · obsidian-vault · legion.
