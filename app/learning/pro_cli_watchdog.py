...
def maybe_recover() -> bool:
    """
    Check if CLI_DOWN flag is active; probe the CLI; clear flag on recovery.
    ...
    """
    try:
        from .pro_router import is_cli_down, clear_cli_down_flag
        ...
        if not probe_cli():
            bg_log(
                "Pro CLI watchdog: CLI still unavailable — continuing API fallback. "
                "Will probe again in 5 min.",
                source="pro_cli_watchdog",
            )
            return False

        # CLI is responding again — clear flag, Pro resumes immediately
        clear_cli_down_flag()
        bg_log(
            "Pro CLI watchdog: CLI RECOVERED ✓ — CLI_DOWN flag cleared. "
