...
| PG migration applied 2026-04-24 | complete | via `railway ssh --service Legion -- python -m app.run_migration`. Four tables verified present on `divine-contentment` Postgres: `agent_quota`, `claude_account_state`, `hive_rounds`, `hive_agent_scores`. `services/legion/app/run_migration.py` added (commit `15d08cd`). Blocker to flipping DUAL_ACCOUNT_ENABLED/LEGION_ENABLED is now cleared. |
| P5 dashboard widget | complete | super-agent `0ce66b9` — "Legion Engineer" added to `AGENT_WORKERS` in `static/agents.html` with ⚔️ avatar and hive-orchestrator description; registered in `_KNOWN_WORKERS` + `_AGENTS` in `agent_status_tracker.py`; new `_fetch_legion_health()` helper in `app/main.py` with 15s cache polls `https://legion-production-36db.up.railway.app/health/detailed` and reconciles widget state — shows "working" when any sub-agent enabled, "idle" otherwise, "sick" if unreachable. Injects `legion_enabled`, `dual_account_enabled`, `l5_enabled`, `agents_registered`, `agents_enabled`, `pg_configured`, `legion_uptime_s`. Verified live: super-agent `/dashboard/agents/status` returns Legion row with `agents_registered=[kimi,gemini_b,ollama,hf,claude_b,chatgpt]`. |
| P6.1 circuit breaker | complete | super-agent `TBD` — `app/circuit.py` adds per-agent CLOSED/HALF_OPEN/OPEN state machine. Hive engine filters shortlist through `circuit.allow()` before fan-out, records `record_success/record_failure` after. OPEN after 5 consecutive errors (configurable via `circuit.error_threshold` in legion_config.yaml), cooldown per agent (Claude-B 300s, Kimi 120s, Gemini-B 60s, Ollama 15s, HF 60s), HALF_OPEN admits one probe. In-memory state (single-replica). 6 unit tests. |
| P6.2 watchPatterns partition | complete | via Railway GraphQL — `Legion.watchPatterns=["services/legion/**"]`; `super-agent`, `VS-Code-inspiring-cat`, `obsidian-vault` all set to `["**","!services/legion/**"]` (gitignore-style: match everything except the legion subdir). Pushes to `services/legion/**` now rebuild only Legion; pushes elsewhere rebuild the other three but not Legion. Eliminates the 3-minute cascade rebuild every push. Fix 2026-04-24: earlier single-negation `["!/services/legion/**"]` was a pure negation that matched nothing → no rebuilds ever fired. |
| P7.1 Kimi CLI re-enabled | complete | super-agent `4d70f93` — uv symlinked to `/usr/local/bin/uv` before Kimi install so `command -v uv` check in install.sh succeeds. `kimi` binary at `/root/.local/bin/kimi` (v1.39.0). |
...
| Kimi agent removed | reverted | super-agent `bfa33b8` — Moonshot moved Kimi CLI login to paid Coding Plan only (`kimi login` returns 402 Payment Required on the free path). Removed all Kimi code; uv install retained. Reinstate from commits 4d70f93 + 2c55033 if you ever buy the plan. |
| 4 free OpenAI-compatible agents | complete | super-agent `TBD` — `groq`, `openrouter`, `cerebras`, `github_models` all share the same OpenAI-compatible client pattern. Each takes its own `*_API_KEY`, `*_MODEL`, `*_BASE_URL` env vars; default to free models on each provider. Tests cover disabled / no-key / happy-path mock paths. Hive is now 9 agents: gemini_b, ollama, hf, claude_b, chatgpt, groq, openrouter, cerebras, github_models. |
| Shared-memory bridge | complete | super-agent `694cbdb` — `app/memory_client.py` calls super-agent `/memory/search` before each hive round and prepends top-K relevant memories (≤2KB context) to every agent's prompt. Configured via `SUPER_AGENT_BASE_URL`. Failure-safe: empty/timeout → original query unchanged. All Legion agents now operate against the same KB context inspiring-cat would have. |
| super-agent ↔ Legion routing | complete | super-agent `TBD` — `app/models/claude.py:_try_legion()` HMAC-signs POST `/v1/respond` and slots into the existing cascade as step 3 (after Claude CLI, Gemini CLI; before Anthropic API). Defensive try/except never raises; falls through silently to API on any error. New `legion_used()` flag mirrors `api_fallback_used()` so the streaming layer can surface "Legion hive used" status to dashboards. Unblocks real failover from inspiring-cat → Legion → Anthropic. |
| Leaked secrets rotated | complete | 2026-04-25 via `railway variables --set` with values generated and consumed inside one PowerShell process (never echoed): `LEGION_API_SHARED_SECRET` and `LEGION_WEBHOOK_SECRET` both replaced with fresh 36-char alphanumeric. Earlier leaked values (visible in transcript via `railway variables` listing) are now dead. n8n's matching `LEGION_WEBHOOK_SECRET` will need to be updated by hand (or via reference syntax `${{Legion.LEGION_WEBHOOK_SECRET}}`) the next time Legion's webhook gets traffic. Currently no traffic flows that path so the mismatch is harmless. |
| Ollama partial setup | partial | 2026-04-25 — `OLLAMA_MODELS=/workspace/legion/ollama` and `OLLAMA_ENABLED=true` set as Railway env vars. **Outstanding:** supervisord.conf still has `autostart=false` for ollama (a P3 safety to prevent boot crashes when env vars were stricter). To finish: change `[program:ollama] autostart=false` → `autostart=true` in services/legion/supervisord.conf, push (will trigger Legion-only rebuild thanks to watchPatterns), then `railway ssh -- ollama pull llama3.2:3b`. Until then `ollama serve` is not running so the OllamaAgent always returns connection-refused. |
