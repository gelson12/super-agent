...
| P6.2 watchPatterns partition | complete | via Railway GraphQL — `Legion.watchPatterns=["/services/legion/**"]`; `super-agent`, `VS-Code-inspiring-cat`, `obsidian-vault` all set to `["!/services/legion/**"]`. Pushes to `services/legion/**` now rebuild only Legion; pushes elsewhere rebuild the other three but not Legion. Eliminates the 3-minute cascade rebuild every push. |
